...
void gFlood::setFloodInitialRGBTexture(unsigned char * data, int width, int height, int nrChan)
{
	...
	m_texture_initial_RGB = createTexture(m_texture_initial_RGB, GL_TEXTURE_2D, 1, width, height, 0, GL_RGB8);
	//err = glGetError();
	...
	//cv::Mat im0 = cv::Mat(1288, 2464, CV_8UC3);
	//glActiveTexture(GL_TEXTURE0);
	//glBindTexture(GL_TEXTURE_2D, m_texture_initial_RGB);
...
void gFlood::jumpFloodCalc()
{
	...
	//std::vector<float> vecdata(640 * 480 * 2);
